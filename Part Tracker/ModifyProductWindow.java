...
public class ModifyProductWindow {
    public ModifyProductWindow(Stage primaryStage,Product selectedProduct){
        Inventory inventory = Inventory.getInstance();
        inventory.resetPartList();
        ObservableList<Part> newPartsList = selectedProduct.getPartsInProductList();
        HBox root = new HBox();
        ...
        TextField idText = new TextField(Integer.toString(selectedProduct.getProductId()));
        idText.setDisable(true);        
        TextField nameText = new TextField(selectedProduct.getName());
        TextField invText = new TextField(Integer.toString(selectedProduct.getInStock()));
        TextField priceText = new TextField(Double.toString(selectedProduct.getPrice()));
        TextField maxText = new TextField(Integer.toString(selectedProduct.getMax()));        
        TextField minText = new TextField(Integer.toString(selectedProduct.getMin()));
...
//Buttons Action Handlers
        ...
        saveBtn.setOnAction((ActionEvent e) -> {
            ...
            if(validationErrorsList.size()>0){
                ...
                Product newProduct = new Product(selectedProduct.getProductId(),nameText.getText(), invValue, prcValue, minValue, maxValue, newPartsList);
                inventory.updateProduct(selectedProduct.getProductId(),newProduct);
                inventory.resetPartList();
